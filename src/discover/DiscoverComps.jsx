...
class RetrieveSearch extends React.Component {
    constructor(props) {
        super(props);
        this.state = { retrieved: [], ids: "", k: 0, sswitch: false };
    }

    shouldComponentUpdate(NextProps, NextState) {

        if (this.props === NextProps && this.state.ids === NextState.ids && this.state.k === NextState.k && this.state.sswitch === NextState.sswitch) {
            return false;
        }
        return true;
    }
    ...
    Switch=()=>{
        ...
        <Grid item>
            <Switch size="small" checked={this.state.sswitch} onChange={() => { this.setState({ sswitch: !this.state.sswitch }); alert("changed") }}  color="primary"/></Grid>
